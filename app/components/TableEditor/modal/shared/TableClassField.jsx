                ...
                {activeDropdown === 'tableClass' && (
                    <ul className={layout.dropdownStyle}>
                        {TABLE_CLASS_SUGGESTIONS.map((cls, idx) => (
                            <li key={idx} className={layout.listItemStyle} onMouseDown={(e) => { e.preventDefault(); onChange(cls.value); setActiveDropdown(null); }}>
                                {cls.label}
                            </li>
                        ))}
                        {TABLE_SCROLL_SUGGESTIONS.map((scroll, idx) => {
                            ...
                            return (
                                <li key={`scroll-${idx}`} className={layout.listItemStyle} onMouseDown={(e) => { e.preventDefault(); onChange(newVal); setActiveDropdown(null); }}>
                                    {scroll.label}
                                </li>
                            );
                        })}
                        <li className={layout.listItemStyle} onMouseDown={(e) => { e.preventDefault(); onChange(''); setActiveDropdown(null); }}>
