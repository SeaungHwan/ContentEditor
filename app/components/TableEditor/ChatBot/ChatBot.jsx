...
// 먼저 존댓말을 반말로 낮춘 뒤에 말버릇을 붙인다. 야돈의 "~양"은 존댓말에 그대로 붙여도 자연스러운
// 별개의 어미 방식이라 이 변환이 필요 없다.
//
// "-하다"/"-되다"로 끝나는 동사는 앞의 어근이 뭐든 반말형이 항상 "-해"/"-돼"로 기계적으로
// 정해지므로(지정합니다→지정해, 적용됩니다→적용돼) 일반 규칙으로 처리한다. 그 외의 존댓말
// 어미는 동사마다 반말형이 불규칙해서(있다→있어, 남다→남아, 붙다→붙어 등) 실제 FAQ 답변에
// 등장하는 것들을 직접 조사해 하나씩 규칙으로 추가했다(chatBotContent.json 전체 답변 기준).
function toCasualImperative(text) {
    return text
        ...
        .replace(/붙습니다([.!?]?)/g, '붙어$1')
        .replace(/열립니다([.!?]?)/g, '열려$1')
        .replace(/지워집니다([.!?]?)/g, '지워져$1')
        .replace(/남겨둡니다([.!?]?)/g, '남겨둬$1')
        .replace(/바뀝니다([.!?]?)/g, '바뀌어$1')
        .replace(/가리킵니다([.!?]?)/g, '가리켜$1')
        .replace(/나타납니다([.!?]?)/g, '나타나$1');
}
