...
// TLD를 화이트리스트로 한정했었으나(뒤에 영단어가 바로 붙는 극히 드문 경우 방지 목적),
// .edu/.us/.ai/.dev 등 목록에 없는 실제 TLD의 이메일이 아예 링크로 안 잡히는 회귀가 있어
// 되돌렸다. {2,}는 어차피 탐욕적으로 문자를 모두 소비하므로 글자 수 제한으로는 애초에
// "뒤에 붙은 단어" 문제를 막을 수 없어 트레이드오프도 없다.
